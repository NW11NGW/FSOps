...
export function NetworkMap({ arcs, airports, homeAirportIcao, selectedPairKey, onSelectPair, className }: NetworkMapProps) {
  ...
  useEffect(() => {
    ...
    // Captured in the effect body rather than read as `markersRef.current` in the cleanup, matching
    // the convention LiveOpsMap and RouteMap already follow. Behaviour is identical - the Map object
    // is created once and `.current` is never reassigned - but reading a ref during cleanup is the
    // pattern that bites when it is not true, so the linter is right to insist and it is not worth
