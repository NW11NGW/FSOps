...
// Every FlightStatus needs an entry, including the ones the logbook itself filters out - the
// Record is exhaustive on purpose, so adding a status to the backend enum breaks the build here
// rather than silently rendering an undefined badge variant. That is not hypothetical: this map
// previously carried a Planned that never existed while missing Skipped, Cancelled and Suspended,
// which the API genuinely returns, so every missed virtual-pilot sector rendered with no variant
// at all.
