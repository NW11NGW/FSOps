...
FSOps opens straight into a full-screen setup wizard whenever no airline exists yet for your machine — on first run, and again any time you delete your airline from the settings danger zone (see the [user guide](user-guide.md#settings)). The wizard has ten steps:
...
8. **MSFS panel** — optionally point FSOps at your MSFS Community folder so it can install the in-game toolbar panel right away. Entirely skippable; set it up (or change it) any time later from Settings.
9. **Online flying** — optionally set your SimBrief Pilot ID and/or VATSIM CID, each with a one-line note on what it unlocks (SimBrief OFP import on the Fly screen; VATSIM flight corroboration and its small bonus). Both are genuinely optional, nothing is checked over the network here, and both can be set or changed later from Settings — see [SimBrief](user-guide.md#simbrief) and [Your VATSIM CID](user-guide.md#your-vatsim-cid). If either already has a value on file (for instance, you re-founded your airline after deleting it), that field is shown locked rather than asked for again.
10. **Review** — optionally add a startup loan (off by default — you have to switch it on and enter an amount and term yourself). The annual rate is shown, not chosen — it's always your playstyle's capped rate, since a brand-new airline has no trading history to price a better one from — with a live monthly payment estimate. The amount itself is capped too: **£250,000** for Casual, **£5,000,000** for True-life; asking for more is refused with an explanation rather than silently trimmed. Review everything you've chosen, and select **Found your airline** to create it.
