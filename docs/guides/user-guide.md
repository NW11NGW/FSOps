...
The Pilots table shows every pilot you employ — your own entry alongside every virtual pilot you've hired — with their status, skill rating, hours flown, monthly salary, and (for virtual pilots) a weekly summary of sectors flown and expected revenue once they have a schedule. **Your own hours flown accrue from your tracked flights** the same way a virtual pilot's do from theirs, so your entry keeps up to date with how much you've actually flown.

The status column reads one of three things, worked out fresh every time you open the page rather than stored anywhere:

- **Flying** — this pilot has a sector in the air right now. For your own entry that means a tracked flight is running; it clears by itself the moment the flight finishes. To see where those flights actually are, use the Dashboard's [live operations map](#the-live-operations-map), which draws every airborne flight, yours and your virtual pilots'.
- **Available** — ready to fly. A virtual pilot with a standing weekly schedule always reads Available however long it's been since their last sector, because their pattern is still rolling and will produce their next one on its own.
- **Inactive** — this pilot has no weekly schedule and hasn't flown anything in a fortnight, so nothing is going to happen for them until you give them one. It's the same fortnight after which [idle skill decay](#skill-landing-quality-and-idle-decay) starts, so the two always agree: a pilot never reads Available next to a decay warning, or Inactive next to a sector they flew yesterday. Give them a schedule and they go back to Available straight away, without having to fly anything first. **Your own entry never reads Inactive** — how long you go between flights is your business, not something the app should have an opinion about.
...
**Copy a day onto other days.** Building eight or nine legs a day, seven days a week, one leg at a time is exactly the tedium this is for. Every day that has legs on it carries a **copy button** in its column header: pick it, tick as many other days as you like, and that day's legs — and its aircraft, since a duty day flies one airframe throughout — land on all of them at the same departure times.

The care is in what happens next, because **a copied day is frequently not legal, and that is the normal case rather than an edge one**. A day is a chain: Monday's legs out of Bristol only work on Wednesday if the aircraft is at Bristol on Wednesday morning, and that depends entirely on what Tuesday did. So before anything is copied, FSOps runs the paste through **the same check that saving runs** and shows you the result in the dialog:

- If it works, it says so.
- If it doesn't, it lists **exactly what breaks, in the same words the save would use** — naming the aircraft, both airports and the day and time, never a generic "that won't work". You can still copy anyway and fix the days in between afterwards, and the dialog says plainly that the week won't save until you have. The choice stays yours; what you don't get is a surprise at save time.
- If a day you're copying onto **already has legs**, it tells you which days and how many legs before you commit, and the button says **Replace** rather than Copy. Nothing is ever quietly overwritten — and because the copy lands in the draft rather than the database, **Discard changes** still puts it all back.

**Rest, duty length and turnaround are enforced, cyclically across the week**: at least **10 hours' rest** between a pilot's duty days, a maximum **13-hour duty day**, and at least **30 minutes'** turnaround between two legs on the same aircraft — the fastest a low-cost narrowbody plausibly turns around, not a suggestion of how tight to cut it; nothing stops you scheduling a longer, more realistic gap for any given leg.

**The week is a loop, and no day is its first.** A saved schedule rolls forever, so Saturday is followed by Sunday and Sunday by Monday, and both of those are ordinary consecutive pairs — a Sunday morning leg is offered from wherever **Saturday's chain leaves the aircraft**, not from wherever the airframe happens to be standing right now, and a leg added on Saturday is checked against Sunday just like any other day. The aircraft's real position only decides where it **joins** the loop, which matters for the very first day you build and not after that.

**A blank week is the hardest part of any planner**, so an empty schedule offers a **Suggest a starter schedule** button. If it can't even try — no routes yet, no aircraft in the fleet, or every aircraft currently reserved for you — it says exactly which of those it is and links straight to the page that fixes it, rather than one generic failure covering every cause. Otherwise it builds outward from morning departures across the **whole week, weekends included**, checking every leg against the real rules as it goes, so what you get is always something you could save immediately and adjust from there — never a proposal that would fail its own validation.

**It tries your whole fleet, best-placed aircraft first.** An aircraft standing where your routes actually depart from is tried before one parked away from your network, and an idle airframe before one already carrying another pilot's week — and if the first choice can't produce a legal week, it moves on to the next rather than giving up. "Couldn't fit a legal starter schedule" now means every eligible aircraft was tried, on every day, and none of them could fly anything.

**It suggests the most profitable week for *that* aircraft, not the same route for everybody.** Among the legs the chosen aircraft can legally fly from where it stands, FSOps picks the one worth the most — using the same economy that eventually pays you: real demand for that city pair, its fare and load factor, fuel priced at the departure airport, and the fees, crew and maintenance the sector actually incurs. All of that is resolved **against the specific airframe**, so seats, weight, burn and cruise speed all count and the same city pair is genuinely worth different money to a 70-seat turboprop and a 180-seat narrowbody. It compares whole *days*, not single sectors: a long-haul pair worth several times more per sector still loses to a short hop that can be flown seven or eight times in the same duty day, because the point is a week that keeps the aircraft working. And a leg another pilot is already flying is worth less to the next one — a city pair's market is finite and shared — so **two pilots are given different weeks whenever there's a real alternative**, and the same pair only comes round again when it genuinely is the only thing worth flying. After it builds, FSOps tells you in one line why it chose what it chose, naming the aircraft, the leg and roughly what a sector of it earns.

None of this loosens anything: it only ranks options the duty, rest, turnaround, range, runway and continuity rules have *already* approved. A leg that would be refused before is still refused however profitable it looks, and the same airline in the same state always gets the same suggestion — nothing here is random.

**A week that doesn't fit is trimmed, not abandoned.** If the weekend won't work — the aircraft is already committed elsewhere on those days, say — you get the five weekdays that do rather than nothing at all. The same is true in reverse: if weekdays are taken and only Saturday is free, that's what you're offered. Nothing is ever bent to make a day fit; a day that can't be flown legally is simply left empty for you.

**It fills the duty day, and it stops because a rule stops it.** There is no leg limit. A generated day keeps taking legs for as long as the rules above keep allowing the next one, so a short sector like EGKK ↔ EGPH now runs seven or eight legs into the evening rather than knocking off at lunchtime, while a long-haul sector may only fit one. Block time is what decides which, and the answer comes from the same check the manual picker uses, leg by leg — nothing is assumed.

**It tells you what stopped it.** Under the line explaining which route it chose, FSOps names the fullest day it built, how many legs that day carries, and the reason it went no further — usually **in the validator's own words**, like *"Duty on Monday runs 13.4 hours, above the 13-hour maximum duty day."* That is the difference between a suggestion you have to take on trust and one you can check: "it stopped because the duty day ran out at 18:36" is an answer, and it tells you there are no more hours to find without changing something.

**Days chain across airports, the way you'd build them by hand.** A generated day no longer has to finish where it started. Monday can end at Edinburgh and Tuesday pick up there the next morning, which is what lets a day take the odd extra sector instead of being rounded down to whole round trips. What is still guaranteed is the thing that actually matters: **the week as a whole ends where it began**, because a standing pattern has to repeat. If the last day would leave the aircraft in the wrong place, that final leg is dropped rather than offered — and when that happens the notice says so plainly instead of blaming the duty rules for a day that still had hours in it. A week that can't be closed at all is not proposed; you're told so and pointed at building it manually.

**Turnarounds vary between 30 and 40 minutes.** Rather than stamping the same gap on every turn, each one is drawn from that range — above the 30-minute minimum, never at or below it — so a generated week reads like an airline rather than a spreadsheet. The variation is fixed rather than random: the same airline in the same state produces the same week every time you ask, so pressing the button twice never gives you two different answers.

Worth knowing: because one pilot now genuinely fills an airframe's day, a second pilot usually needs a **second aircraft**. Where the old four-leg suggestion left whole afternoons free for someone else to use, a filled day does not.

**A saved schedule is a pattern that rolls forever, not a plan that goes stale.** Your aircraft move — that's the point of flying them — so a week that was perfectly valid when you saved it will often start from an airport the aircraft isn't standing at any more, or one it's in the air on its way back from. That never stops you saving, and it never invalidates the schedule: the pattern keeps repeating, and it starts flying again the moment the aircraft is back in position. FSOps tells you once, when you save, with a note naming the aircraft and where it currently is — a notice, not an error. **The note distinguishes the two cases that actually differ**, because a week is a closed loop: if the aircraft is standing at *any* airport the week departs from, that airport's own leg simply flies and everything after it lines up by itself, so the note says it picks up there rather than sending you to fetch an aircraft that doesn't need fetching. Only when the aircraft is somewhere **no leg departs from** does it name the airport to get it back to and offer the two ways of doing that — flying it there yourself, or [repositioning it from the Fleet page](#moving-an-aircraft-to-another-airport) for the standard fee. What it won't do any more is refuse the save, and it will never refuse *another* pilot's save over an aircraft that isn't in their week. Individual legs are still judged honestly at the time they're due: a leg the aircraft genuinely can't reach is still [skipped or cancelled with its reason recorded](#the-wall-clock-economy-flying-while-youre-away), exactly as before.

**If a pattern does stop flying, the Pilots page tells you.** There is one way a rolling schedule can go quiet indefinitely: the aircraft ends up parked at an airport that **no leg in the pattern departs from**. Nothing in the schedule can move it from there, so every occurrence is recorded as missed, week after week, and nothing else would ever say so. When that happens the Pilots roster shows a notice naming the pilot and the aircraft, where it is versus where the pattern starts, and the two ways out: fly it back yourself, or [reposition it from the Fleet page](#moving-an-aircraft-to-another-airport) for the standard fee. It clears itself the moment the aircraft is back — there is nothing to dismiss.

It is deliberately quiet about every other out-of-position case, because those repair themselves. If the aircraft is standing at somewhere the week *does* depart from, the pattern picks up at that leg and re-synchronises on its own, so there is nothing to tell you. An aircraft in the air, or in maintenance, is likewise left alone: neither is stuck.
...
- **Costs** — your operating costs split into **fixed** (leases, salaries, insurance, loan repayments) and **variable** (fuel, landing, handling, parking, passenger charges, turnaround, maintenance, crew, plus cancellation fees and aircraft repositioning when there have been any), because they behave completely differently: fixed costs are owed whether or not you fly, variable ones only bite when you do. Alongside them sits **Revenue**: ticket revenue, your [online-flying bonus](#the-online-flying-bonus) if you've earned one, and any aircraft sale proceeds — each on its own line, so a one-off sale or an online bonus never quietly inflates what your fares look like they're earning.
- **Routes** — profit and loss per route: sectors flown, ledger-derived revenue, cost and profit for the window, so you can see which routes in your network are actually worth flying rather than guessing from the fare alone. A route's revenue includes the online-flying bonus earned on its sectors, so a route's profit here is exactly what those sectors' **Net** figures add up to in your [flight logbook](#the-flight-logbook).
...
The Dashboard in full. The sim clock at the top left runs off the simulator's own time rather than your machine's; the KPI row and the reputation card beneath it are read back from the ledger and your last few sectors, never stored separately. In the "Live operations" card, nothing is airborne, so the map says so in a card that folds away to a pill.

**This capture predates the ATC button, so read the VATSIM part of it with that in mind.** It was taken when the controller layer was always on: the only button above the map is **Show VATSIM traffic**, the controller sectors are already drawn across Europe, and the **ATC coverage** card below the map is listing what is in view. Today there are two buttons — **Show VATSIM ATC** beside **Show VATSIM traffic** — and both layers start hidden, so a new airline sees neither the sectors on the map nor the ATC coverage card until it asks for them.
...
A row of headline figures sits above everything else: **Sectors flown**, **Avg on-time**, **Avg load factor**, **Fleet utilisation**, and **Flown online** — the share of your eligible sectors that were corroborated on the network, counted against those eligible (see [The online-flying bonus](#the-online-flying-bonus)). None of them ever shows a zero it can't stand behind: a window with nothing to measure reads "Not measured", an airline with no aircraft reads "No aircraft", and a period with no eligible sectors reads "Not tracked". Below that, six tabs:
