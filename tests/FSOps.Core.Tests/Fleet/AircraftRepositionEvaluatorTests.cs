...
public class AircraftRepositionEvaluatorTests
{
    ...
    // The repositioning fee's fixed-versus-variable classification is asserted where the split
    // actually happens - FinanceEndpointsTests.Costs_CountsRepositioningAsAVariableCost - rather
    // than against a helper. It used to be checked here through LedgerCostClassifier, which turned
    // out to be a second copy of the Finances page's own category lists that nothing called; the
    // page had inlined the same decision long before. Asserting on the endpoint's real response
    // means the test fails if the money lands in the wrong bucket, which is the thing that matters.
}
