...
public class FlightLogbookAndTrackEndpointTests
{
    ...
    public async Task Logbook_IncludesAttemptedSectors_AndExcludesOnesThatNeverLeftTheGate()
    {
        ...
        // rather than accurate. Planned/Skipped/Cancelled/Suspended never flew at all, so they are
        // not flying done and do not belong here.
        using var ctx = await RouteTestContext.CreateAsync();
        ...
        SeedFlight(ctx, route, aircraft.Id, pilot.Id, FlightStatus.Planned, Base.AddHours(9));
