...
public class BackupEndpointsIntegrationTests : IAsyncLifetime
{
    ...
    public async Task Download_LeavesNoTemporaryFileBehind()
    {
        ...
        // response has finished, which is a moment after the client has the last byte. Poll rather
        // than sleep, so this asserts the file really goes away instead of guessing how long to wait.
        var scratchDirectory = PendingRestore.BackupsDirectory(_dataDirectory);
        for (var attempt = 0; attempt < 60 && Directory.GetFiles(scratchDirectory, "working-*").Length > 0; attempt++)
        {
            await Task.Delay(50);
        }
